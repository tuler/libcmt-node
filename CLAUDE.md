...
- `lib/index.js` (CJS, the real implementation) — argument conversion (0x-hex strings / `Uint8Array` / `bigint` ⇄ Buffers), the `Rollup` class wrapping the native one, the `run()` handler loop, and the EVM-ABI codec helpers (`decodeAdvance`, `encodeNotice`, `encodeVoucher`, `encodeDelegateCallVoucher`) that mirror libcmt's `codec.c` Output1..Output4 envelopes byte-for-byte, built on `ox` (the only runtime JS dependency; cross-checked byte-for-byte against codec.c in the tests). Loads the addon via `node-gyp-build` (prefers `build/Release`, falls back to `prebuilds/`).
