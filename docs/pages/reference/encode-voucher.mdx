...
`encodeVoucher` is a pure-JS helper that returns the EVM-ABI encoded bytes (a CALL voucher in an `Output2(bytes32[2],bytes)` envelope). Emit them with [`emitOutput`](/reference/emit-output) to add the voucher to the outputs merkle tree (provable).
...
The output index comes from [`emitOutput`](/reference/emit-output)'s return value: the voucher's position among all provable outputs ever emitted by the application. Off-chain components use it to fetch the voucher's proof for execution.

## Returns

`Buffer` — the encoded voucher, ready to pass to [`emitOutput`](/reference/emit-output).
