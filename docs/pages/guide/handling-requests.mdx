...
- **Advance** — an input submitted on-chain. It can change application state and produce provable outputs ([vouchers](/reference/encode-voucher) and [notices](/reference/encode-notice)).
...
1. The application calls [`waitForInput`](/reference/wait-for-input), telling the machine whether it **accepts or rejects** the previous request.
...
Advance handlers receive the request with its **raw, undecoded payload**. Parse it with [`decodeAdvance`](/reference/decode-advance) to get the input metadata and inner payload:

```ts twoslash
import { Rollup, decodeAdvance } from '@tuler/node-libcmt';
...
await rollup.run({
    advance(request, rollup) {
        ...
        if (advance.payload.length > 1024) {
...
`run` is a thin convenience. If you prefer explicit control — custom dispatch, graceful shutdown, request batching — call [`waitForInput`](/reference/wait-for-input) directly:
...
for (;;) {
    ...
    switch (request.type) {
        case 'advance':
            rollup.emitOutput(encodeNotice(decodeAdvance(request.payload).payload));
            accept = true;
            break;
        case 'inspect':
