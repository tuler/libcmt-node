...
When the input list is exhausted, the next [`waitForInput`](/reference/wait-for-input) call fails and [`run`](/reference/run) rejects — that is your test's natural end-of-session signal.
...
import { decodeAdvance, encodeNotice } from '@tuler/node-libcmt';
