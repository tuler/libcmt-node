...
class Rollup {
    ...
    constructor() {
        this.#native = new binding.Rollup();
    }
     ...
     */
    finish({ accept = true } = {}) {
        const request = this.#native.finish(accept);
    ...
    emitNotice(payload) {
        return Number(this.#native.emitNotice(toBytes(payload, 'payload')));
    }

    /** Emit a report (raw bytes, not part of the outputs merkle tree). */
    emitReport(payload) {
        this.#native.emitReport(toBytes(payload, 'payload'));
    }
    ...
    progress(value) {
        this.#native.progress(value);
    }

    /** Perform a generic IO request to the given domain. */
    gio({ domain, id }) {
        return this.#native.gio(domain, toBytes(id, 'id'));
    }

    loadMerkle(file) {
        this.#native.loadMerkle(String(file));
    }

    saveMerkle(file) {
        this.#native.saveMerkle(String(file));
    }

    resetMerkle() {
        this.#native.resetMerkle();
    }
...
module.exports = { Rollup, ADDRESS_LENGTH, U256_LENGTH };
